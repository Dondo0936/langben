...
export default function PromptsWithFolder() {
  ...
  const handleExport = async (mode: "latest" | "all") => {
    ...
    try {
      ...
      a.href = url;
      a.download = `vet-prompts-${mode}-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      URL.revokeObjectURL(url);
      capture("prompts:bulk_export", { mode });
    } catch {
      toast.error("Xuất prompt thất bại. Thử lại.");
    } finally {
  ...
  const { data: count } = api.prompts.count.useQuery(
    { projectId },
    {
      enabled: !!projectId && !promptNameFromRoute,
      trpc: {
        context: {
          skipBatch: true,
        },
  ...
  return (
    <Page
      headerProps={{
        ...
        help: {
          description:
            "Quản lý và version prompt. Sửa trên UI hoặc SDK. Lấy bản production qua SDK.",
        },
        actionButtonsRight: (
          <>
