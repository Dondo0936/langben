...
Cài đặt project và tổ chức: khóa ingest, thành viên, cấu hình điểm, kết nối LLM, host console.
...
1. **Khóa API** — public/secret để SDK và OTLP gửi vào console `:3000`. Demo: `pk-lf-vet-demo` / `sk-lf-vet-demo`. Không gửi `ANTHROPIC_API_KEY` vào Vết.
2. **Tên host** — base URL ingest của console này (`http://localhost:3000` khi local).
3. **Cấu hình điểm** — khai báo tên score (`helpful`) trước khi chấm hàng loạt.
