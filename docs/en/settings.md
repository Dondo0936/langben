...
1. **API keys** — public/secret so the SDK and OTLP can send into console `:3000`. Local demo: `pk-lf-vet-demo` / `sk-lf-vet-demo`. Rotate those before exposing `:3000`. Do not send `ANTHROPIC_API_KEY` to Vết.
